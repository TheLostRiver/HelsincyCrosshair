...
**当前版本 / 引擎**: `v4.0.2` / Unreal Engine 4.27
...
- **COD 风格命中反馈**：支持普通命中、爆头、击杀、单实例刷新、多实例叠加、扭转、缩放、针状线条、法线震动、伤害强度响应、基础准星可见性策略，以及 `SpriteDualLayer` Core/Glow 贴图路径。
- **SpriteDualLayer HitMarker 调校**：完整 X 贴图可使用 `WholeSpriteShake` 整体震动；需要四臂独立冲击时可切到 `PerArmQuadrantShake` 并配置单臂贴图；最短可感知显示时长用于避免贴图命中反馈一闪而过。
...
- `Content/HitmarkerImage/T_HC_HitMarker_Core.uasset`：`SpriteDualLayer` 完整 X 命中准星核心贴图。
- `Content/HitmarkerImage/T_HC_HitMarker_Glow.uasset`：`SpriteDualLayer` 完整 X 命中准星辉光贴图；四臂独立模式需要项目侧另配单臂贴图。
...
命中反馈配置支持 `SingleInstance` 与 `MultiInstance` 两种模式。`SingleInstance` 是默认 COD 风格路径，可选择程序化 `LegacyGeometry` 或贴图驱动的 `SpriteDualLayer`；`SpriteDualLayer` 下的 `WholeSpriteShake` 使用完整 X 贴图，`PerArmQuadrantShake` 使用单臂贴图绘制四条独立手臂。详细字段说明见 [中文用户手册](Docs/UserManual_CN.md) 和 [集成指南](Docs/Integration-Guide.md)。
...
| v4.0.2 发布说明 | [Docs/Release-Notes-v4.0.2.md](Docs/Release-Notes-v4.0.2.md) | 所有用户 | HitMarker SpriteDualLayer 闪烁修复、运动模式、默认几何调校和发布包说明 |
...
- 当前发布版本为 `v4.0.2`；`v4.0.0` 是首次公开 Release，成熟版首发口径不代表此前存在公开 v1/v2/v3 Release。
- 当前发布包以 Unreal Engine 4.27 验证；如果你要在 UE5 项目中使用，请先在目标引擎版本完成项目侧编译与运行验证。
- `v4.0.2` 修复 SpriteDualLayer HitMarker 闪烁，新增 sprite 运动预设，并调轻默认 HitMarker 几何参数；详见 [CHANGELOG.md](CHANGELOG.md)。
