...
两个模块的公共头会暴露 GameplayTag、DeveloperSettings、Engine 基础类型，以及准星目标检测使用的 AI 阵营接口类型。因此模块的 `Build.cs` 会把这些公共 API 依赖声明在 `PublicDependencyModuleNames` 中，避免宿主项目或外部插件包含公共头时依赖宿主模块的偶然间接依赖。
...
6. 准星组件 BeginPlay 时按本地玩家归属决定 Pending / Active / Deactivated 状态，并决定是否从 `DefaultCrosshairAsset` 加载默认配置。
7. 伤害指示器组件 BeginPlay 时按本地玩家归属决定 Pending / Active / Deactivated 状态，并决定是否从 `DefaultDataAsset` 加载默认配置。
8. HUD 或 Bridge 绘制入口在每帧绘制时查找组件与 Subsystem。

本地玩家守卫会把“当前 owner 是否通过本地真人玩家检查”和“默认资产 / 预设是否已经完成一次性初始化”分成两个状态。这样 Controller 延迟就位时组件可以保持轻量 Pending；AI、远端玩家或非本地 owner 会停用组件并清理 owner eligibility；同一 Pawn 后续重新变成本地真人玩家时可以重新激活，而不会因为一次性初始化状态被保留而重复加载默认资产。
...
准星组件同一时间只保留一个活动 `FTraceHandle`。如果异步目标检测超过配置的等待帧数，组件会放弃该活动请求并允许发起新的 trace；旧 trace 即使稍后回调，也必须通过 `FTraceHandle` 身份校验后才允许更新命中状态。这保证过期回调不会覆盖较新的目标颜色或错误清空当前目标。
...
目标检测结果由当前活动 `FTraceHandle` 保护。回调到达时，只有 handle 仍等于组件记录的活动请求，才会执行命中处理或清空目标；被超时逻辑丢弃的旧请求会被当作 stale callback 忽略。这一点对多人、低帧率或碰撞查询偶发延迟场景尤其重要，因为目标高亮必须反映最新的准心方向，而不是较早一帧的 trace 结果。
